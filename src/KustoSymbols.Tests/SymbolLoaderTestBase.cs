...
namespace Tests
{
    ...
    public class SymbolLoaderTestBase
    {
        ...
        protected static void AssertEqual(ClusterSymbol expected, ClusterSymbol actual)
        {
            ...
            Assert.HasCount(expected.Members.Count, actual.Members, "cluster members count");
        ...
        protected static void AssertEqual(GraphModelSymbol expected, GraphModelSymbol actual)
        {
            ...
            Assert.HasCount(expected.Edges.Count, actual.Edges, "graph model edge steps");
            for (int i = 0; i < expected.Edges.Count; i++)
            {
                Assert.AreEqual(expected.Edges[i].Body, actual.Edges[i].Body, "graph model edge step");
            }

            Assert.HasCount(expected.Nodes.Count, actual.Nodes, "graph model node steps");
